...
use cosmwasm_std::{
    ...
    StdError, Timestamp, Uint128, WasmMsg, SubMsg, Addr
};
...
fn happy_days_end_poll() {
    ...
    let mut creator_info = mock_info(
        VOTING_TOKEN,
    ...
    let execute_res = execute(deps.as_mut(), creator_env.clone(), creator_info.clone(), msg).unwrap();
