...
use cosmwasm_std::{
    attr, coins, from_binary, to_binary, Coin, CosmosMsg, Decimal, Deps, DepsMut, Env, Response,
...
fn proper_initialization() {
    ...
    let msg = init_msg();
    let env = mock_info(TEST_CREATOR, &coins(2, VOTING_TOKEN));
...
fn poll_not_found() {
    ...
    let res = query(deps.as_ref(), QueryMsg::Poll { poll_id: 1 });
...
fn fails_create_poll_invalid_title() {
    ...
    let env = mock_info(VOTING_TOKEN, &vec![]);
    match execute(deps.as_mut(), env.clone(), msg) {
        Ok(_) => panic!("Must return error"),
...
fn happy_days_create_poll() {
    let mut deps = mock_dependencies(20, &[]);
    mock_init(deps.as_mut());
    let env = mock_info_height(VOTING_TOKEN, &vec![], 0, 10000);
...
fn fails_end_poll_before_end_height() {
    ...
    let res = query(deps.as_ref(), QueryMsg::Poll { poll_id: 1 }).unwrap();
    ...
    let env = mock_info_height(TEST_CREATOR, &vec![], 0, 10000);
    let execute_res = execute(deps.as_mut(), env, msg);
...
fn happy_days_end_poll() {
    ...
    mock_init(deps.as_mut());
    let mut creator_env = mock_info_height(
        VOTING_TOKEN,
    ...
    let env = mock_info(VOTING_TOKEN, &[]);
    let execute_res = execute(deps.as_mut(), env.clone(), msg.clone()).unwrap();
    ...
    let mut env = mock_info_height(TEST_VOTER, &[], POLL_START_HEIGHT, env.block.time.seconds());
    ...
    let execute_res = execute(deps.as_mut(), env.clone(), msg).unwrap_err();
    match execute_res {
        StdError::GenericErr { msg, .. } => assert_eq!(msg, "Poll is not in passed status"),
        _ => panic!("DO NOT ENTER HERE"),
    }

    creator_env.message.sender = (TEST_CREATOR);
...
fn expire_poll() {
    ...
    let env = mock_info_height(TEST_VOTER, &[], POLL_START_HEIGHT, env.block.time.seconds());
    ...
    let execute_res = execute(deps.as_mut(), creator_env.clone(), msg);
    match execute_res {
    ...
    let msg = ExecuteMsg::ExpirePoll { poll_id: 1 };
    let _execute_res = execute(deps.as_mut(), creator_env.clone(), msg).unwrap();
...
fn end_poll_zero_quorum() {
    let mut deps = mock_dependencies(20, &coins(1000, VOTING_TOKEN));
    mock_init(deps.as_mut());
    let mut creator_env = mock_info_height(VOTING_TOKEN, &vec![], 1000, 10000);
    ...
    let env = mock_info(VOTING_TOKEN, &[]);
    execute(deps.as_mut(), env, msg.clone()).unwrap();
...
fn end_poll_quorum_rejected() {
    ...
    let msg = create_poll_msg("test".to_string(), "test".to_string(), None, None);
    let mut creator_env = mock_info(VOTING_TOKEN, &vec![]);
    let execute_res = execute(deps.as_mut(), creator_env.clone(), msg.clone()).unwrap();
    assert_eq!(
        execute_res.attributes,
        vec![
...
fn end_poll_nay_rejected() {
    ...
    let mut deps = mock_dependencies(20, &[]);
    mock_init(deps.as_mut());
    let mut creator_env = mock_info(VOTING_TOKEN, &coins(2, VOTING_TOKEN));
    ...
    let env = mock_info(TEST_VOTER_2, &[]);
    let msg = ExecuteMsg::CastVote {
        poll_id: 1,
        vote: VoteOption::No,
...
fn withdraw_voting_tokens() {
    ...
    let voter_addr = TEST_VOTER.to_string();
    poll_voter_store(deps.storage, 1u64)
        .save(
        ...
        .unwrap();
    poll_voter_store(deps.storage, 2u64)
        .save(
            &voter_addr.as_str().as_bytes(),
            &VoterInfo {
                vote: VoteOption::Yes,
    ...
    let _ = execute(deps.as_mut(), env, msg).unwrap();
...
fn fails_withdraw_voting_tokens_no_stake() {
    ...
    let env = mock_info(TEST_VOTER, &coins(11, VOTING_TOKEN));
    let msg = ExecuteMsg::WithdrawVotingTokens {
        amount: Some(Uint128::from(11u128)),
    };
...
fn fails_cast_vote_twice() {
    ...
    let env = mock_info_height(VOTING_TOKEN, &coins(2, VOTING_TOKEN), 0, 10000);
    ...
    let env = mock_info_height(TEST_VOTER, &[], 0, 10000);
    let execute_res = execute(deps.as_mut(), env.clone(), msg).unwrap();
    assert_cast_vote_success(TEST_VOTER, amount, 1, VoteOption::Yes, execute_res);
...
fn fails_insufficient_funds() {
    ...
    let msg = init_msg();
    let env = mock_info(TEST_VOTER, &coins(2, VOTING_TOKEN));
    let init_res = instantiate(deps.as_mut(), env, msg).unwrap();
...
fn fails_staking_wrong_token() {
    ...
    let env = mock_info(VOTING_TOKEN.to_string() + "2", &[]);
    let res = execute(deps.as_mut(), env, msg);
...
fn share_calculation_with_voter_rewards() {
    ...
    // create poll
    let env = mock_info(VOTING_TOKEN, &coins(2, VOTING_TOKEN));
    let msg = create_poll_msg("test".to_string(), "test".to_string(), None, None);
    let execute_res = execute(deps.as_mut(), env.clone(), msg.clone()).unwrap();
    assert_create_poll_result(
        1,
...
fn distribute_voting_rewards() {
    ...
    let env = mock_info_height(TEST_VOTER, &[], poll_end_height, env.block.time.seconds());
    let msg = ExecuteMsg::EndPoll { poll_id: 1 };
...
fn distribute_voting_rewards_only_to_polls_in_progress() {
    ...
    poll_indexer_store(deps.storage, &PollStatus::InProgress)
        ...
        .unwrap();
    poll_indexer_store(deps.storage, &PollStatus::Passed)
...
fn test_staking_and_voting_rewards() {
    ...
    let res = query(deps.as_ref(), QueryMsg::State {}).unwrap();
    ...
    let res = query(deps.as_ref(), QueryMsg::Staker { address: (ALICE) }).unwrap();
    let response: StakerResponse = from_binary(&res).unwrap();
    assert_eq!(
        response,
        StakerResponse {
...
fn test_query_shares() {
    ...
    let voter_0_addr = "staker0000".to_string();
    let voter_1_addr = "staker0001".to_string();
    let voter_2_addr = "staker0002".to_string();
...
fn happy_days_cast_vote_with_snapshot() {
    ...
    let env = mock_info_height(TEST_VOTER_2, &[], end_height - 9, env.block.time.seconds());
    let execute_res = execute(deps.as_mut(), env.clone(), msg).unwrap();
    ...
    let env = mock_info_height(TEST_VOTER_3, &[], end_height - 8, env.block.time.seconds());
    let execute_res = execute(deps.as_mut(), env.clone(), msg).unwrap();
...
fn fails_end_poll_quorum_inflation_without_snapshot_poll() {
    ...
    let env = mock_info_height(VOTING_TOKEN, &[], 0, env.block.time.seconds());
    let _execute_res = execute(deps.as_mut(), env.clone(), msg.clone()).unwrap();
...
fn stake_voting_tokens_multiple_lock_end_weeks() {
    ...
    let env = mock_info(VOTING_TOKEN, &[]);
    let execute_res = execute(deps.as_mut(), env, msg.clone());
...
fn test_unstake_before_claiming_voting_rewards() {
    ...
    let env = mock_info_height(TEST_VOTER, &[], env.block.height, env.block.time.seconds());
    let _res = execute(deps.as_mut(), env.clone(), msg).unwrap();
    ...
    let msg = ExecuteMsg::WithdrawVotingTokens { amount: None };
    let mut env = mock_info_height(TEST_VOTER, &[], env.block.height, env.block.time.seconds());
