...
use cosmwasm_std::{from_binary, Coin, Deps, DepsMut, Env, HumanAddr, StdError, Uint128};
...
fn proper_initialization() {
    ...
    let config: PenaltyConfig = read_config(deps.storage).unwrap();
    assert_eq!(
        config,
        PenaltyConfig {
            owner: HumanAddr::from("penalty_owner"),
...
fn test_ema_math() {
    ...
    let msg = init_msg();
    let env = mock_info("addr0000", &[]);
    instantiate(deps.as_mut(), env, msg).unwrap();
    let ema = get_ema(deps.as_mut(), 50, FPDecimal::from(100u128)).unwrap();
...
fn test_mint_actions() {
    ...
    assert_eq!(
        response.attributes,
        vec![attr("penalty", FPDecimal::from_str("-4.2").unwrap())]
    );
...
fn test_redeem_actions() {
    ...
    assert_eq!(
        response.attributes,
        vec![attr("penalty", FPDecimal::from_str("0").unwrap())]
    );
